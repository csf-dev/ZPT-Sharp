...
namespace CraigFowler.Web.ZPT.Tal
{
  ...
  public class RepeatVariable : IEnumerator
  {
    ...
    /// </returns>
    public string GenerateLetterReference(int numericIndex)
    {
    ...
    /// </example>
    /// </remarks>
    private void GenerateLetterReference(int numericIndex, int characterPosition, ref StringBuilder output)
    {
       ...
       * 
       * We divide numericIndex by ALPHABET_LENGTH to the power of the characterPosition and then discard
       * any digits after the decimal point.  This both normalises the numericIndex to the context of the
       ...
       */
      truncatedIndex = Math.Truncate((double) numericIndex / Math.Pow(ALPHABET_LENGTH, characterPosition));
