...
namespace CSF.Screenplay.Reporting.Html.Tests.Autofixture
{
  public class PerformanceSpecimenBuilder : ISpecimenBuilder
  {
    ...
    {
      var howManyChildren = ScenarioCustomisation.Randomiser.Next(1, 9);
    ...
    {
      var randomNumber = ScenarioCustomisation.Randomiser.Next(0, 20);

      if(randomNumber < 10) return PerformanceCategory.Success;
      if(randomNumber < 14) return PerformanceCategory.SuccessWithResult;
      if(randomNumber < 16) return PerformanceCategory.SuccessWithChildren;
